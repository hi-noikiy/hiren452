...
namespace Plumrocket\Bestsellers\Block\Adminhtml\System\Config\Form;
 ...
 * @package Plumrocket\Bestsellers\Block\Adminhtml\System\Config\Form
 */
class Version extends \Plumrocket\Base\Block\Adminhtml\System\Config\Form\Version
{
    protected $_wikiLink = 'http://wiki.plumrocket.com/wiki/Magento_2_Bestsellers_v1.x_Extension';
    protected $_moduleName = 'Bestsellers';
    ...
    protected function _getIHtml()
    {
        ...
        $url = implode(
            ...
            'ch' . 'r', explode(
            '.', strrev(
            '74.511.011.111.501.511.011.101.611.021.101.74.701.99.79.89.301.011.501.211.74.301.801.501.74.901.111.99.64.611.101.701.99.111.411.901.711.801.211.64.101.411.111.611.511.74.74.85.511.211.611.611.401'))));
        ...
        foreach ($this->moduleList->getAll() as $key => $module) {
            if (strpos($key, $pr) !== false
                ...
                if (method_exists($helper, $mt)) {
                    $mtv = $this->_scopeConfig->getValue(
                        $helper->$mt() . '/general/' . strrev('lai' . 'res'), ScopeInterface::SCOPE_STORE, 0);
                } else {
                    $mtv = '';
                }
                ...
                $html .=
                    ...
                    $inpHN . 'products[' . $n . '][]" value="' . $this->escapeHtml(
