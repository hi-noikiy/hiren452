    ...
    window.prnewsletterPopupClass = function (_settings) {
        ...
        this.show = function () {
            ...
            if (id in loaded && $('#newspopup_up_bg_'+id+' .newspopup-up-form').is(':visible') == false) {
                ...
                // Activate Data Privacy (GDPR) checkboxes (knockout components)
                var $checkboxesBlock = $('.prgdpr-consent-checkboxes-block', loaded[id]);
                if ($checkboxesBlock.length) {
                    $checkboxesBlock.trigger('contentUpdated');
                    if (typeof $checkboxesBlock.applyBindings === 'function') {
                        $checkboxesBlock.applyBindings();
                    }
                }
