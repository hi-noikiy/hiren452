...
class Adminhtml extends Main
{
     ...
     * @param Repository                               $viewRepository
    ...
    {
        return (bool)$this->getConfig($this->_configSectionId . '/integration/mailchimp/enable');
    }
        ...
        if (!$this->_mailchimp) {
            if ($this->isMaichimpEnabled()) {
                $this->_mailchimp = new Mcapi(
                    trim($this->_encryptor->decrypt($this->getConfig($this->_configSectionId . '/integration/mailchimp/key'))),
                    true
                );
            }
        }
    ...
    public function checkIfHtmlToImageInstalled()
    {
        ...
        if (null === $this->_checkIfHtmlToImageInstalledResult) {
            ...
            if ($which = shell_exec('which wkhtmltoimage')) { // @codingStandardsIgnoreLine
                $this->_checkIfHtmlToImageInstalledResult = trim($which);
            } else {
                $path = $this->_cache->load($cacheKeyName);
                if ($path) {
                    $this->_checkIfHtmlToImageInstalledResult = shell_exec("find $path -name \"wkhtmltoimage\""); // @codingStandardsIgnoreLine
