...
class Sizechart extends \Magento\Framework\View\Element\Template
{
    ...
    protected static $_categoriesSC = [];
     ...
     * @param \Magento\Cms\Model\Template\FilterProvider $templateFilter
     ...
     * @param \Plumrocket\SizeChart\Helper\Data $helper
        ...
        if (!$this->hasData('content')) {
            ...
            if ($this->helperData->moduleEnabled()) {
                if ($sizechart = $this->getSizeChart()) {
                    $this->setData(
                        'content',
                        $this->templateFilter->getPageFilter()->filter($sizechart->getContent())
                    );
                }
            }
        }
    ...
    protected function _getPlSizeChartIdByProduct($product)
    {
        if ($product->getPlSizeChart() == -1) {
            return null;
        } elseif ($sizechartByRules = $this->_getPlSizeChartByRules($product, true)) {
            return $sizechartByRules;
        } elseif ($product->getPlSizeChart()) {
    ...
    protected function _getPlSizeChartIdByCategory($category)
    {
        ...
        if ($category->getPlSizeChart() == -1) {
            $sizeChartId = -1;
        } elseif ($category->getPlSizeChart()) {
    ...
    {
        if (($sizechart = $this->getSizeChart()) && !empty($sizechart->getButtonLabel())) {
            return $sizechart->getButtonLabel();
        }

        return $this->helper->getDefaultButtonLabel();
    }
    ...
    {
        $cacheKey = self::SIZE_ATTR_IDS_CACHE_KEY . $this->encryptor->hash(
            implode(',', $keys),
        ...
        $attributeInfo = $this->attrCollectionFactory->create()
            ->addFieldToFilter('attribute_code', $arrayForFilter);
