...
class Bankday extends Data
{
     ...
     * @param \Plumrocket\Estimateddelivery\Model\OrderItemFactory $orderItemFactory
     */
    ...
    public function prepareDate($s)
    {
        if ($s !== null && !is_int($s)) {
            ...
            if ($ts === -1 || $ts === false) {
                throw new \Exception(
                    'Unable to parse date/time value from input: ' . var_export($s, true)
                );
            }
        } else {
            $ts = $s;
    ...
    /**
     * Retrieve date + n bank days
     *
     * @param string $type
     * @param string $start date in store (Magento) time zone
     * @param int    $days
     * @param null   $format
