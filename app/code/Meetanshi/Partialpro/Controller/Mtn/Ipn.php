...
namespace Meetanshi\Partialpro\Controller\Mtn;
...
class Ipn extends Action\Action
{
    ...
    public function execute()
    {
        if ($this->helper->isModuleEnabled()) {
            ...
            try {
                ...
                $result->setData(['success' => false, "message" => "Payment fail"]);

                $params = $this->getRequest()->getParams();

                if (isset($params) && $params['Reference'] != '' && $params['ResponseCode'] != '') {

                    if (strpos($params['Reference'], '-') !== false) {

                        //Partial Installment code
                        $installmentArrId = explode("-", $params['Reference']);
                        ...
                        if ($sizeofArr > 1) {
                            ...
                            if ($order->getId()) {
                                ...
                                foreach ($installmentArrId as $installmentId) {
                                    ...
                                    $this->setInstallmentSuccessData($order, $partialPayment, $installment, '  ');
                        ...
                        $order = $this->orderFactory->create()->loadByIncrementId($params['Reference']);
                        if ($order->getIncrementId()) {
                            $payment = $order->getPayment();
                            if ($params['ResponseCode'] == 01 || $params['ResponseCode'] == 529 || $params['ResponseCode'] == 100 || $params['ResponseCode'] == 1000) {
                                $payment->setAdditionalInformation('response_code', $params['ResponseCode']);
                                $payment->setAdditionalInformation('response_message', $params['ResponseMessage']);
                                $order->save();

                                $partialPaymentItem = $this->partialPaymentCollection->create()
                                    ->addFieldToFilter('order_id', $params['Reference'])->getFirstItem();

                                if ($partialPaymentItem->getId()) {
                                    $partialPaymentId = $partialPaymentItem->getPartialPaymentId();
                                    $installmentItem = $this->partialInstallmentCollection->create()
                                        ->addFieldToFilter('partial_payment_id', $partialPaymentId)->getFirstItem();

                                    if ($installmentItem->getId()) {
                                        if ($installmentItem->getInstallmentStatus() != 2) {

                                            $installmentItem->setPaymentMethod('orangeivory')
                    ...
                    $result->setData(['success' => true, "message" => "Payment successfully."]);
                }
                return $result;
            } catch (\Exception $e) {
    ...
    protected function setInstallmentSuccessData($order, $partialPayment, $installment, $transactionid)
    {
        if ($installment->getId()) {
            $installment->setPaymentMethod('mtn')
                ->setInstallmentStatus(2)
                ->setInstallmentPaidDate(date("Y-m-d H:i:s"))
