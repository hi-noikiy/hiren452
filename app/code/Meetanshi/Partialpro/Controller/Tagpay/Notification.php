...
class Notification extends Action\Action
{
    ...
    public function execute()
    {
        ...
        try {
            if ($params['purchaseref'] != '') {

                if (strpos($params['purchaseref'], '-') !== false) {

                    //Partial Installment code

                    if ($params['status'] == 'OK') {

                        $installmentArrId = explode("-", $params['purchaseref']);
                        ...
                        if ($sizeofArr > 1) {
                            ...
                            if ($order->getId()) {
                                ...
                                foreach ($installmentArrId as $installmentId) {
                                    ...
                                    $this->setInstallmentSuccessData($order, $partialPayment, $installment, $tranid);
                    ...
                    //Simple Order Place Code

                    $order = $this->orderFactory->create()->loadByIncrementId($params['purchaseref']);
                    ...
                    if ($params['status'] == 'OK') {
                        ...
                        if (array_key_exists("paymentref", $params)) {
                            $payment->setAdditionalInformation('paymentref', $params['paymentref']);
                        }
                        ...
                        $order->addStatusHistoryComment($params['status'], Order::STATE_PROCESSING)->setIsCustomerNotified(true);

                        $order->save();
                        $transaction->save();

                        $partialPaymentItem = $this->partialPaymentCollection->create()
                            ->addFieldToFilter('order_id', $params['purchaseref'])->getFirstItem();
                    ...
                    if ($params['status'] == 'NOK') {
                        ...
                        $payment->save();
                        $this->orderSender->notify($order);
                        $order->addStatusHistoryComment($params['error'], Order::STATE_CANCELED)->setIsCustomerNotified(true);
                        $order->save();
    ...
    protected function setInstallmentSuccessData($order, $partialPayment, $installment, $transactionid)
    {
        if ($installment->getId()) {
            $installment->setPaymentMethod('tagpay')
                ->setInstallmentStatus(2)
                ->setInstallmentPaidDate(date("Y-m-d H:i:s"))
