    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->helperData->isModuleEnabled()) {
            ...
            if ($this->helperData->getPartialProductSet($quote->getId())) {
                ...
                $order->setData('partial_installment_fee', $quote->getPartialInstallmentFee());
                $order->setData('partial_pay_now', $quote->getPartialPayNow());
                $order->setData('partial_pay_later', $quote->getPartialPayLater());
                $order->setData('partial_max_installment', $quote->getPartialMaxInstallment());
                ...
                try {
                    ...
                    $maxInstallment = $quote->getPartialMaxInstallment();
                    ...
                    $modelPartialpayment->setOrderId($order->getIncrementId())
                        ...
                        ->setPaidAmount($quote->getPartialPayNow())
                        ->setRemainingAmount($quote->getPartialPayLater())
                        ...
                        ->setSurchargeAmount($quote->getPartialInstallmentFee());
                    ...
                    if ($method->getCode() == 'ravepayment') {
                        $getAmountBase =  $this->helperData->convertCurrency($order->getBaseCurrencyCode(),$order->getOrderCurrencyCode(), $quote->getPartialPayNow());
                        $order->setData('total_paid', $order->getTotalPaid() + $quote->getPartialPayNow());
                    ...
                    $modelInstallments->setPartialPaymentId($modelPartialpayment->getPartialPaymentId())
                        ->setInstallmentAmount($quote->getPartialPayNow())
                    ...
                    $remainingAmountArr = [];
                    for ($i = 1; $i < $quote->getPartialMaxInstallment(); $i++) {
