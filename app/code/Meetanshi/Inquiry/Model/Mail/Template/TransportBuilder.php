...
namespace Meetanshi\Inquiry\Model\Mail\Template;
...
class TransportBuilder extends TBuilder
{
    ...
    public function addAttachment($fileString, $filename, $fileType = 'application/octet-stream')
    {
        ...
        try {
            if (method_exists($this->message, 'createAttachment')) {
                $this->message->createAttachment(
                    file_get_contents($fileString, false, stream_context_create($arrContextOptions)),
                ...
                if (version_compare($version, '2.3.3') >= 0) {
                    ...
                    $attachmentPart->setContent(file_get_contents($fileString, false, stream_context_create($arrContextOptions)))
                        ->setType($fileType)
                    ...
                    $attachment = new \Zend\Mime\Part(file_get_contents($fileString, false, stream_context_create($arrContextOptions)));
